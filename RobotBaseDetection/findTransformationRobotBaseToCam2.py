...
def camera_robot_calibration_show_clicks(base_img, camera_matrix=None, distortion_coefficients=None):
    ...
    if detected is not None and len(detected) > 0:
        ...
        print(f"Detected {len(detected)} circles. Click these 5 points in order:")
    else:
    ...
    print("  3) Y1 (front/upper of Y pair)")
    print("  4) Y2 (back/lower of Y pair)")
    print("  5) Center (big center circle)\n")
    ...
    window_name = "Select Circles (click 5)"
    ...
    # Return both clicked points and matched centers (or None)
    return {
        "clicks": selected_points,
        "matched_centers": matched_centers if len(matched_centers) > 0 else None,
...
def main():
    ...
    result = camera_robot_calibration_show_clicks(img)
    print("\nResult dict returned from function:")
    print(result)
